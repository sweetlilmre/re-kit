...
def dump(status):
    out = ["# Generated by toolkit/pascal/ratchet.py and observe.py. The lock is",
    ...
    for key in sorted(status.get("routine", {})):
        ...
        for f in ("matched", "target", "achieved", "reason"):
            if row.get(f) not in (None, ""):
                v = row[f]
                out.append("%s = %s" % (f, v if isinstance(v, int) else toml_str(v)))
        out.append("")
    for key in sorted(status.get("observation", {})):
        row = status["observation"][key]
        out.append("[observation.%s]" % toml_str(key))
        for f in ("harness", "tier", "outcome", "achieved", "observer", "date",
                  "confirmed_at", "fingerprint", "against", "note"):
            if row.get(f) not in (None, ""):
                out.append("%s = %s" % (f, toml_str(row[f])))
        out.append("")
    return "\n".join(out).rstrip("\n") + "\n"
