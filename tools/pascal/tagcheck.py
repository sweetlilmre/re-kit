...
# **A LABELLED COMMAND NAMES ITS TOOL FOR THE SAME REASON**, and it is the same
# exemption one step on. `Build:  build.py build.toml TP1S1` in a harness header
# is the line that tells a reader how to build the thing they are reading; the
# tool's name is not a measurement, it is half the instruction, and removing it
# leaves a command that cannot be typed.
#
# Labelled, and only labelled. A bare mention of a tool in prose is apparatus
# and stays flagged -- what earns the exemption is the shape of a command the
# reader is meant to run, which is what the label marks.
COMMAND = re.compile(r'^[ \t]*(?:Build|Run|Usage|Invoke|Rebuild):[ \t]', re.M)
