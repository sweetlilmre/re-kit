...
| `pascal/` (30) | the instruments that measure a rebuild -- `routines`, `units`, `objcheck`, `blockcmp`, `linkcmp`, `mapcmp`, `linkorder`, `dgroup`, `spans`, `coverage`, `artefact`; the ones that read a binary before you write any Pascal -- `survey`, `rtl`, `x87`, `emit`, `prologue`; `build` which makes what the rest measure; the register's own -- `plan`, `ratchet`, `observe`, `markers`, `marker`, `register`, `staged`; and the source checks `paslint`, `asmaudit`, `undeclared`, `shared_asm`, `codegen` |
