...
- **A DOS source line may not exceed 127 characters, COMMENTS INCLUDED.** Turbo Pascal answers `Error 11: Line too long` and refuses the unit, and it counts a comment exactly as it counts code. Four comment lines at 129 to 134 characters failed a build with 21 errors -- 3 real and 18 the cascade of missing `.TPU` that follows the first three.

  **No static check sees this.** The encoding auditor, the brace-depth scan, the tag checker and the stripper all passed those same files, because none of them measures a line's length; the compiler is the only reader that does. It is the one defect class in a comment edit that only a build can report, which is the argument for a build being in the gate rather than run occasionally.

  When wrapping to fix one, walk the wrap point LEFT until the continuation does not begin with a citation. The obvious wrap put an address at a line start, where a stripper trims it and leaves a sentence opening with `and`.
